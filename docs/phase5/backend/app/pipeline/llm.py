...
def _pass2_system_prompt() -> str:
    return (
        ...
        "- Every action_items entry MUST include a non-empty title (use description if needed).\n"
    )
...
def _action_title_from_raw(item: dict) -> str:
    """Derive a non-empty title; LLM sometimes omits title or uses legacy 'action' only."""
    ...
    if evidence and evidence.lower() != "not identified":
        return evidence[:300]
    return ""
...
def _normalize_actions(raw_items: list, meeting_start_iso: Optional[str], timezone_name: Optional[str]) -> list[ActionItem]:
    ...
    for item in raw_items or []:
        if not isinstance(item, dict):
            continue
        title = _action_title_from_raw(item)
        if not title:
            continue
        due = anchor_relative_due_date(
            str(item.get("due_date") or "Not identified"),
