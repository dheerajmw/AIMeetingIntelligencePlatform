...
def index_meeting(db: Session, meeting: Meeting) -> None:
    ...
    if existing:
        ...
        existing.workspace_id = meeting.workspace_id
...
def keyword_search(db: Session, query: str, limit: int = 20, workspace_id: int | None = None) -> list[dict]:
    q = query.strip()
    if not q:
        return []
    ...
    rows = db.execute(
        ...
        {"query": q, "limit": limit * 3 if workspace_id else limit},
    ).mappings().all()

    hits: list[dict] = []
    for row in rows:
        meeting_id = int(row["meeting_id"])
        doc = db.get(MeetingSearchDocument, meeting_id)
        ...
        hits.append(
            {
                ...
                "title": doc.title if doc else f"Meeting #{meeting_id}",
...
def semantic_search(db: Session, query: str, limit: int = 20, workspace_id: int | None = None) -> list[dict]:
    stmt = select(MeetingSearchDocument)
    if workspace_id is not None:
        stmt = stmt.where(MeetingSearchDocument.workspace_id == workspace_id)
...
def search_meetings(
    ...
    if mode == "semantic":
        return semantic_search(db, query, limit=limit, workspace_id=workspace_id)
    return keyword_search(db, query, limit=limit, workspace_id=workspace_id)


def reindex_all_ready(db: Session, workspace_id: int | None = None) -> int:
    stmt = select(Meeting).where(Meeting.status == MeetingStatus.READY, Meeting.deleted_at.is_(None))
    if workspace_id is not None:
