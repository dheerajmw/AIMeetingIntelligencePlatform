...
import { SampleShowcase, type SampleItem } from './SampleShowcase'
...
function App() {
  ...
  async function trySample(sample: SampleItem) {
    ...
    setBusy(true)
    try {
      const r = await fetch(`${apiBase}/samples/${sample.id}/process`, { method: 'POST' })
      if (!r.ok) throw new Error(await r.text())
    ...
    <div className="page">
      <header className="header">
        <div>
          ...
          <h1>Phase 4 — Real-time Intelligence</h1>
          <p className="subtitle">
            Batch upload (Phase 3) plus live streaming with draft transcript/summary and finalize-to-final outputs.
          </p>
        </div>
        <div className="api">
      ...
      <div className="exportGrid" style={{ marginBottom: 12 }}>
        <button className={`btnGhost ${mode === 'batch' ? 'activeTab' : ''}`} onClick={() => setMode('batch')}>
          Batch upload
        </button>
        <button className={`btnGhost ${mode === 'live' ? 'activeTab' : ''}`} onClick={() => setMode('live')}>
          Live meeting
        </button>
      </div>

      {mode === 'live' ? (
        <LivePanel apiBase={apiBase} />
      ) : (
      <>
      <SampleShowcase apiBase={apiBase} busy={busy} onTrySample={trySample} />
      <main className="grid">
